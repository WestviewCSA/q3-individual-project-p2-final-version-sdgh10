...
//            
//            if (useStack) {
//                System.out.println("Using Stack-based approach");
//                path = solver.solveWithStack();
//            }
//            else if (useQueue) {
...
//                                        IncompleteMapException, 
//                                        IllegalMapCharacterException {
//        layers = readMazeFile(filename);  // Store layers
//        findStartAndGoal();
//    }
//    
//    public ArrayList<String[][]> readMazeFile(String filename) throws IncorrectMapFormatException, 
...
//    public void findStartAndGoal() {
//        for (int l = 0; l < layerCount; l++) {
//            String[][] grid = layers.get(l);  // FIXED: Use stored layers
//            for (int r = 0; r < rows; r++) {
//                for (int c = 0; c < cols; c++) {
//                    if (grid[r][c].equals("W")) {
//                        start = new int[]{r, c, l};
...
//        }
//        String cell = layers.get(layer)[row][col];
//        return !cell.equals("@");  // Everything except walls is valid
//    }
//    
//    public int teleport(int currentLayer) {
//        return (currentLayer + 1) % layerCount;
...
//                
//                if (isValid(newRow, newCol, newLayer)) {
//                    String cell = layers.get(newLayer)[newRow][newCol];  // FIXED
//                    
//                    if (cell.equals(".") || cell.equals("W") || cell.equals("$")) {
//                        Position next = new Position(newRow, newCol, newLayer, current);
//                        if (!visited.contains(next.getKey())) {
...
//    
//    
// // QUEUE SOLVER - Breadth-First Search (BFS)
// // Following the spec: enqueue and dequeue operations
//    public ArrayList<Position> solveWithQueue() {
...
//    	Queue<Position> queue = new LinkedList<>();
//     
//     // ENQUEUE Wolverine's start position
//    	Position startPos = new Position(start[0], start[1], start[2]);
//    	queue.add(startPos);  // enqueue
//    	visited.add(startPos.getKey());
//     
////     // Debug output
////    	System.out.println("Start: (" + start[0] + "," + start[1] + ") in layer " + start[2]);
////    	System.out.println("Goal: (" + goal[0] + "," + goal[1] + ") in layer " + goal[2]);
////     
//     // Directions: North, South, East, West (in that order as spec says)
//    	int[][] directions = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};  // N, S, E, W
//     
//    	while (!queue.isEmpty()) {
//         // DEQUEUE the next location
//         Position current = queue.poll();  // dequeue
//         
//         // For each direction in order: North, South, East, West
//         for (int[] dir : directions) {
//             int newRow = current.getRow() + dir[0];
//             int newCol = current.getCol() + dir[1];
//             int newLayer = current.getLayer();
...
//                 }
//                 
//                 // Handle regular walkable tiles (., W, $)
//                 if (cell.equals(".") || cell.equals("W") || cell.equals("$")) {
//                     Position next = new Position(newRow, newCol, newLayer, current);
...
//                         
//                         // Check if this space holds the coin
//                         if (next.isGoal()) {
//                       //      System.out.println("Found goal! Path length: " + reconstructPath(next).size());
//                             return reconstructPath(next);
//                         }
//                     }
//                 }
//                 // Handle walkway - teleport to next layer
//                 else if (cell.equals("|")) {
//                     int teleportedLayer = teleport(newLayer);
//                     
//                     // Debug: show teleportation
//                   //  System.out.println("Teleporting from layer " + newLayer + " to layer " + teleportedLayer + 
//                    //                  " at (" + newRow + "," + newCol + ")");
//                     
//                     // Check if teleport destination is within bounds and not a wall
//                     if (teleportedLayer >= 0 && teleportedLayer < layerCount) {
//                         String destCell = layers.get(teleportedLayer)[newRow][newCol];
//                         
//                         if (!destCell.equals("@")) {
//                             Position teleported = new Position(newRow, newCol, teleportedLayer, current);
//                             String teleportedKey = teleported.getKey();
//                             
//                             if (!visited.contains(teleportedKey)) {
//                                 visited.add(teleportedKey);
//                                 queue.add(teleported);  // enqueue
//                                 
//                                 // Check if teleported space holds the coin
//                                 if (teleported.isGoal()) {
//                             //        System.out.println("Found goal after teleport! Path length: " + reconstructPath(teleported).size());
//                                     return reconstructPath(teleported);
//                                 }
//                             }
//                         }
...
//     }
//     
//   //  System.out.println("No path found. Visited " + visited.size() + " positions");
//     return null;  // No path found
// }
//    
//    
// // ULTRA FAST OPTIMAL SOLVER - No 3D arrays, maximum speed
//    public ArrayList<Position> solveOptimal() {
...
//        }
//        
//        // Flattened visited array - 1D boolean (NO 3D ARRAYS!)
//        int totalCells = layerCount * rows * cols;
//        boolean[] visited = new boolean[totalCells];
//        
//        // Pre-calculate multipliers for fast index calculation
//        int rowMultiplier = cols;
//        int layerMultiplier = rows * cols;
//        
//        // Array-based queue using primitive ints for maximum speed
//        int maxSize = totalCells;
//        int[] queueRows = new int[maxSize];
//        int[] queueCols = new int[maxSize];
//        int[] queueLayers = new int[maxSize];
//        int[] parentIndex = new int[maxSize];
//        int front = 0;
//        int rear = 0;
//        
//        // Pre-calculate teleport layers once (avoid repeated modulo)
//        int[] teleportMap = new int[layerCount];
//        for (int i = 0; i < layerCount; i++) {
//            teleportMap[i] = (i + 1) % layerCount;
//        }
...
//            if (currentRow == goal[0] && currentCol == goal[1] && currentLayer == goal[2]) {
//                return reconstructPathFast(queueRows, queueCols, queueLayers, parentIndex, front);
//            }
//            
...
//                            queueCols[rear] = newCol;
//                            queueLayers[rear] = currentLayer;
//                            parentIndex[rear] = front;
//                            rear++;
//                        }
//                    } 
//                    // Walkway - teleport to next layer
//                    else {
//                        int newLayer = teleportMap[currentLayer];
//                        int newIndex = newLayer * layerMultiplier + newRow * rowMultiplier + newCol;
//                        if (!visited[newIndex]) {
...
//    }
//
//    // Fast path reconstruction using primitive arrays
//    public ArrayList<Position> reconstructPathFast(int[] rows, int[] cols, int[] layers, int[] parentIndex, int endIndex) {
//        ArrayList<Position> path = new ArrayList<>();
//        
//        // First, count how many steps in the path
//        int length = 0;
//        int tempIndex = endIndex;
//        while (tempIndex != -1) {
//            length++;
...
//        }
//        
//        // Build path array from end to start
//        Position[] tempPath = new Position[length];
//        int pos = length - 1;
//        int currentIndex = endIndex;
//        while (currentIndex != -1) {
...
//        }
//        
//        // Add to ArrayList in correct order
//        for (int i = 0; i < length; i++) {
//            path.add(tempPath[i]);
//        }
//        
...
//        
//        ArrayList<String[][]> solutionLayers = new ArrayList<>();
//        for (String[][] original : layers) {  // FIXED: Use stored layers
//            String[][] copy = new String[rows][cols];
//            for (int i = 0; i < rows; i++) {
//                System.arraycopy(original[i], 0, copy[i], 0, cols);
//            }
